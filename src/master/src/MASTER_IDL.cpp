...
class MASTER_IDL_update : public yarp::os::Portable {
public:
  bool _return;
  void init();
...
class MASTER_IDL_start : public yarp::os::Portable {
public:
  bool _return;
  void init();
  virtual bool write(yarp::os::ConnectionWriter& connection);
...
bool MASTER_IDL_update::write(yarp::os::ConnectionWriter& connection) {
...
bool MASTER_IDL_update::read(yarp::os::ConnectionReader& connection) {
...
bool MASTER_IDL_quit::read(yarp::os::ConnectionReader& connection) {
  yarp::os::idl::WireReader reader(connection);
  if (!reader.readListReturn()) return false;
  if (!reader.readBool(_return)) {
    reader.fail();
...
void MASTER_IDL_quit::init() {
  _return = false;
}

bool MASTER_IDL_start::write(yarp::os::ConnectionWriter& connection) {
  yarp::os::idl::WireWriter writer(connection);
  if (!writer.writeListHeader(1)) return false;
...
bool MASTER_IDL_start::read(yarp::os::ConnectionReader& connection) {
  yarp::os::idl::WireReader reader(connection);
  if (!reader.readListReturn()) return false;
  if (!reader.readBool(_return)) {
    reader.fail();
...
bool MASTER_IDL::update() {
  bool _return = false;
  MASTER_IDL_update helper;
  helper.init();
  if (!yarp().canWrite()) {
    yError("Missing server method '%s'?","bool MASTER_IDL::update()");
  }
...
bool MASTER_IDL::start() {
  bool _return = false;
  MASTER_IDL_start helper;
  helper.init();
  if (!yarp().canWrite()) {
    yError("Missing server method '%s'?","bool MASTER_IDL::start()");
  }
  bool ok = yarp().write(helper,helper);
  return ok?helper._return:_return;
}
...
bool MASTER_IDL::read(yarp::os::ConnectionReader& connection) {
  yarp::os::idl::WireReader reader(connection);
  reader.expectAccept();
...
std::vector<std::string> MASTER_IDL::help(const std::string& functionName) {
